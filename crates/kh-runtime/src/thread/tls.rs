//! Guest↔host TLS: `TPIDR_EL0` + per-thread host snapshot.
//!
//! Guest code runs with TPIDR → freestanding [`GUEST_TLS_MAGIC`] block; every
//! host entry restores the host TPIDR saved at prepare. Snapshots live in
//! [`crate::host_slot`] and are mirrored into the guest block (A1) so enter can
//! restore host TPIDR + alt SP without `gettid` / map locks.
//!
//! Guest block layout (must match `kh-libsystem`):
//! ```text
//! 0 magic  8 errno  16 pthread_self  24 host_tpidr  32 alt_top  40 tsd_vals
//! ```
//! Host owns 24/32; freestanding owns `tsd_vals`.
#![allow(unsafe_code)]
