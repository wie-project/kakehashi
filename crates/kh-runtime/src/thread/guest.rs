//! Guest threads: host pthread spawn + `bsdthread_*` helpers.
//!
//! - `bsdthread_register` stores the libpthread start trampoline VA.
//! - `bsdthread_create` spawns a host thread and jumps with Darwin
//!   `_pthread_start` regs (`x0`–`x3`).
//! - `bsdthread_terminate` returns to a saved host stack, then ends **this**
//!   host thread only (never exit on the guest stack).
//! - Join completion is published from the host stack so joiners never unmap
//!   a live stack. Worker teardown uses raw `SYS_exit` (glibc `pthread_exit`
//!   unwinds through guest frames). Live spawn: Linux aarch64 only.
