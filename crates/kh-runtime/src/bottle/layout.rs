...
/// Guest-relative directories the user must copy from macOS 26+ Apple Silicon.
pub const REQUIRED_PREFIX_DIRS: &[&str] = &["bin", "sbin", "usr/bin"];

/// Returns `true` when `bin`, `sbin`, and `usr/bin` each contain at least one
/// Mach-O file (a real macOS prefix, not an empty skeleton or host-ELF leftovers).
#[must_use]
pub fn bottle_has_macos_prefix(root: &Path) -> bool {
    REQUIRED_PREFIX_DIRS
        .iter()
        .all(|rel| dir_contains_macho(&root.join(rel)))
}

/// Short reminder + copy instructions for a bottle missing the macOS prefix.
#[must_use]
pub fn macos_prefix_hint(root: &Path) -> String {
    format!(
        "bottle is missing the macOS prefix directories (bin, sbin, usr/bin).\n\
         \n\
         Copy these folders from a macOS 26+ Apple Silicon Mac into the bottle:\n\
           /bin     → {root}/bin/\n\
           /sbin    → {root}/sbin/\n\
           /usr/bin → {root}/usr/bin/\n\
         \n\
         Intel Macs and older macOS releases are not supported.\n\
         See README.md (\"Manual Binaries Transfer\").",
        root = root.display()
    )
}

fn dir_contains_macho(dir: &Path) -> bool {
    let Ok(entries) = fs::read_dir(dir) else {
        return false;
    };
    for ent in entries.flatten() {
        let path = ent.path();
        if looks_like_macho_file(&path) {
            return true;
        }
    }
    false
...
fn looks_like_macho_file(path: &Path) -> bool {
    use std::io::Read;
...
mod tests {
    ...
    #[test]
    fn prefix_ready_requires_macho_in_each_dir() {
        let root = temp_root("prefix");
        materialize(&root).expect("materialize");
        assert!(!bottle_has_macos_prefix(&root));
        // Thin Mach-O magic (MH_MAGIC_64 LE).
        let macho = b"\xcf\xfa\xed\xfe";
        fs::write(root.join("bin/rm"), macho).expect("bin");
        fs::write(root.join("sbin/mount"), macho).expect("sbin");
        fs::write(root.join("usr/bin/xcrun"), macho).expect("usr/bin");
        assert!(bottle_has_macos_prefix(&root));
