//! Trap backend: residual Darwin `svc` → `brk`, and freestanding hypercall entry.
//!
//! Production: freestanding libSystem → `kh_hypercall_entry` (alt stack + NEON
//! tramp + [`kh_trampoline_dispatch`]). Residual unpatched `svc` is rewritten to
//! `brk #IMM` and handled via `SIGTRAP` (not a second product path).
...
pub fn patch_svc_to_brk(regions: &mut [MappedRegion]) -> Result<usize, TrapError> {
    ...
    for region in regions.iter_mut() {
        ...
        // Prefer instruction-section ranges (full-TEXT false-positives in data).
        ...
        region.prot = restore;
        // Flush only patched sites (pure file maps stay demand-paged).
...
// Freestanding hypercall entry (`kh_hypercall_entry`): host alt stack preferred
// (join must not run Rust on a guest stack that can be unmapped).
//
// NEON: Darwin `svc` preserves Q0–Q31 — full Q* + FPCR/FPSR saved on the guest
// prolog frame *before* any `bl` (AAPCS64 clobbers). Dispatch via `TRAMP_BYTES`
// (second NEON frame on host/alt stack). ABI: x0–x6 + # in x7 → TrampRet x0/x1.
// Guest frame (640 B): [0] x29,x30; [16..] x0–x7; [80] pad; [96] q0–q31; [608] fpcr,fpsr.
#[cfg(all(target_arch = "aarch64", target_os = "linux"))]
std::arch::global_asm!(
    r#"
    .text
