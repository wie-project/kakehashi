...
//! [`crate::kh_core::helpers::KH_HELPER_HEAP_STATS_ON`] and seeds soft env. Guest `setenv` /
...
use crate::kh_core::sys::{self, SYS_MMAP, SYS_MUNMAP};
...
/// [`crate::kh_core::helpers::KH_HELPER_HEAP_STATS_ON`]. Digs and benches set
...
fn soft_env_seed_heap_flag() {
    ...
    let existing = unsafe { crate::dylib::libsystem_c::posix::getenv(key.as_ptr().cast()) };
    if !existing.is_null() {
        return;
    }
    // SAFETY: helper id matches kh-runtime; no guest buffer.
    let on = unsafe { sys::helper0(crate::kh_core::helpers::KH_HELPER_HEAP_STATS_ON) };
    if on <= 0 {
        return;
    }
    let val = b"1\0";
    let _ = unsafe { crate::dylib::libsystem_c::posix::setenv(key.as_ptr().cast(), val.as_ptr().cast(), 1) };
}

fn soft_env_heap_stats_requested() -> bool {
    let key = b"KAKEHASHI_HEAP_STATS\0";
    // SAFETY: key is NUL-terminated static.
    let p = unsafe { crate::dylib::libsystem_c::posix::getenv(key.as_ptr().cast()) };
    if p.is_null() {
        return false;
    }
...
pub unsafe extern "C" fn malloc(size: usize) -> *mut c_void {
    ...
    crate::dylib::libcxx::iostream::ensure_iostream_vtables();
...
pub unsafe extern "C" fn calloc(count: usize, size: usize) -> *mut c_void {
    ...
    if !p.is_null() && total > 0 {
        ...
        unsafe {
            crate::dylib::libsystem_c::stdio::bzero(p, total);
        }
    }
    p
}
...
pub unsafe extern "C" fn realloc(ptr: *mut c_void, size: usize) -> *mut c_void {
    ...
    unsafe {
        let _ = crate::dylib::libsystem_c::stdio::memcpy(fresh, ptr, old_size.min(size));
        free(ptr);
    }
    fresh
}
