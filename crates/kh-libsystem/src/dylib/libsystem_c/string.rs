...
pub(crate) unsafe extern "C" fn memset_pattern16(
    ...
    if len == 0
        || b.is_null()
        || b.addr() < crate::dylib::libsystem_c::stdio::PAGEZERO_END
        || pattern16.is_null()
        || pattern16.addr() < crate::dylib::libsystem_c::stdio::PAGEZERO_END
    {
        return;
    }
...
pub(crate) unsafe extern "C" fn strdup(s: *const c_char) -> *mut c_char {
    ...
    let n = unsafe { crate::dylib::libsystem_c::stdio::strlen(s) };
...
pub(crate) unsafe extern "C" fn strcpy(dst: *mut c_char, src: *const c_char) -> *mut c_char {
    ...
    unsafe {
        let n = crate::dylib::libsystem_c::stdio::strlen(src).saturating_add(1);
...
pub(crate) unsafe extern "C" fn strncpy(
    ...
    if src.is_null() {
        unsafe {
            let _ = crate::dylib::libsystem_c::stdio::memset(dst.cast(), 0, n);
        }
        return dst;
    }
    unsafe {
...
pub(crate) unsafe extern "C" fn strcat(dst: *mut c_char, src: *const c_char) -> *mut c_char {
    ...
    unsafe {
        let end = dst.add(crate::dylib::libsystem_c::stdio::strlen(dst));
...
pub(crate) unsafe extern "C" fn memset_s(s: *mut c_void, smax: usize, c: c_int, n: usize) -> c_int {
    ...
    if n > smax {
        if !s.is_null() && smax > 0 {
            let _ = unsafe { crate::dylib::libsystem_c::stdio::memset(s, c, smax) };
        }
        return 22;
    }
    if n > 0 && !s.is_null() {
        let _ = unsafe { crate::dylib::libsystem_c::stdio::memset(s, c, n) };
    }
    0
}
...
pub(crate) unsafe extern "C" fn __memmove_chk(
    ...
    unsafe { crate::dylib::libsystem_c::stdio::memmove(dst, src, len) }
}
...
pub(crate) unsafe extern "C" fn __memset_chk(
    ...
    unsafe { crate::dylib::libsystem_c::stdio::memset(dst, c, len) }
}
