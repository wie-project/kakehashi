...
fn file_from_fd(fd: c_int) -> *mut FileStub {
    ...
    let raw = unsafe { crate::kh_core::heap::malloc(core::mem::size_of::<FileStub>()) };
    if raw.is_null() {
        errno::set_errno(12);
        return core::ptr::null_mut();
    }
...
pub(crate) unsafe extern "C" fn fopen(path: *const c_char, mode: *const c_char) -> *mut c_void {
    ...
    let fd = unsafe { crate::dylib::libsystem_c::posix::kh_open_impl(path, flags, creat_mode) };
...
pub(crate) unsafe extern "C" fn getdelim(
    ...
    if buf.is_null() || cap == 0 {
        cap = 128;
        buf = unsafe { crate::kh_core::heap::malloc(cap).cast::<c_char>() };
        if buf.is_null() {
            errno::set_errno(12);
            return -1;
        }
    ...
    loop {
        ...
        if len.saturating_add(2) > cap {
            ...
            let new_buf = unsafe { crate::kh_core::heap::realloc(buf.cast(), new_cap).cast::<c_char>() };
            if new_buf.is_null() {
                errno::set_errno(12);
                return -1;
            }
...
pub(crate) unsafe extern "C" fn ftell(stream: *mut c_void) -> i64 {
    ...
    let fd = unsafe { (*f).fd };
    // SEEK_CUR = 1
    unsafe { crate::dylib::libsystem_c::posix::lseek(fd, 0, 1) }
}
