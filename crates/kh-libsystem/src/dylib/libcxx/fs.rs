...
use crate::dylib::libcxx::string::{
    string_assign_bytes, string_clear, string_copy_from, string_data, string_len,
};
use crate::dylib::libsystem_c::posix::{mkdir, stat, unlink};
...
pub(crate) unsafe extern "C" fn fs_create_directory(p: *const c_void, ec: *mut c_void) -> bool {
    ...
    if rc == 0 {
        ...
        let err = crate::kh_core::errno::get_errno();
        if err == 17 {
            // EEXIST
            set_error_code(ec, 0);
            true
...
pub(crate) unsafe extern "C" fn fs_create_symlink(
    ...
    // Ensure parent directory of the link exists (`mkdir -p` via mkpath_np).
    {
            ...
            && slash > 0
        {
            ...
            parent[slash] = 0;
            let _ = unsafe {
                crate::dylib::misc::ld_misc::mkpath_np(parent.as_ptr().cast::<c_char>(), 0o755)
            };
        }
    }
    let rc = unsafe { crate::dylib::libsystem_c::posix::symlink(target, link) };
    if rc == 0 {
        set_error_code(ec, 0);
        true
    } else {
