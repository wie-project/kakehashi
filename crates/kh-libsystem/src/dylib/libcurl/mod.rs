...
//! network I/O uses host [`crate::kh_core::helpers::KH_HELPER_TLS_CONNECT`]:
...
use crate::kh_core::helpers::KH_HELPER_TLS_CONNECT;
use crate::kh_core::heap::{free, malloc, realloc};
use crate::dylib::libsystem_c::posix::{close, read};
use crate::dylib::libsystem_c::stdio::{memcpy, strlen, write};
...
fn dup_cstr(src: *const c_char) -> Option<*mut c_char> {
    ...
    if src.addr() < crate::dylib::libsystem_c::stdio::PAGEZERO_END {
        return None;
    }
...
fn maybe_decode_claim_gzip(user_hdrs: &[u8], body: &mut PostBody) -> bool {
    ...
    let Some((p, n)) = crate::dylib::libz::inflate_to_malloc(src) else {
...
pub(crate) unsafe extern "C" fn curl_slist_append(
    ...
    if list.is_null() || list.addr() < crate::dylib::libsystem_c::stdio::PAGEZERO_END {
        return node.cast();
    }
