...
| Heap mid-size → arena (drop ≥256 mmap) | freestanding | **landed** (2026-08). G5 blunt `malloc≥256→page mmap` caused ~15k `mmap`/`munmap` per `-cc1` on g4-mini. Restored size-class arena for `<16 KiB`; page-aligned mmap only for `≥PAGE` + `posix_memalign` / `operator new(align)`. **G5+LTO still green.** Analyzer: total crossings ~**153k → ~4.6k** (~33×); `mmap_ok` per cc1 ~**15k → ~20**; child `host_dispatch` ~**0.9s → ~0.2s**. Wall on g4-mini still ~6.5s (start tax dominates: warm `clang --version` ~0.9s × nested processes). Next: cheaper nested map/start |
| Load path dig + thin-bytes cache | kh-loader | **landed** (2026-08). Opt-in `KAKEHASHI_LOAD_TIMING=1` dumps phase ms before guest entry. Warm `clang --version` (fat CLT binary ~277 MiB arm64 slice): was **~0.92 s** load — **bind ~505 ms** (second full `fs::read` + chained) + **map_main ~366 ms**. Fix: retain arm64 thin bytes from first parse; `fill_exports` / chained bind reuse them (no re-read). After: **~0.52 s** (`parse_main` ~230 ms, `mmap_main` ~160 ms, bind ~60 ms, `patch_svc` ~46 ms). g4-mini full driver **~6.5 s → ~3.6 s** (~1.8×). Fat `file_slice_offset` preserved for map. Next levers: cheaper parse of fat/large images; skip or bound `patch_svc` scan on huge `__TEXT`; process model |
| Single-buffer map + patch_svc | kh-runtime / loader | **landed** (2026-08, **general** — any guest). (1) Keep full container `file_bytes` from parse; `map_image_bytes` fills segments from RAM (no second disk pass). (2) `patch_svc_to_brk`: scan only `file_bytes` of RX regions; **mprotect RW only if hits** (fixtures with embedded `svc` still patched; product tools usually 0 hits). (3) I-cache flush only over image content, not BSS tail. After (Docker warm): `clang --version` load **~0.42–0.47 s** (`parse` ~170–210 ms still dominant, `mmap_main` ~86–90 ms, bind ~60 ms, `patch_svc` ~44 ms scan). g4-mini flto still ~**3.6 s**, product PASS; `puts_hello` / `write_exit` PASS. Next: process model / fewer nested `kh`; optional parse mmap without full `Vec` |
| Demand-paged parse (`mmap` container) | kh-runtime / loader | **landed** (2026-08, **general**). `MappedFile` + `FileImage`: open Mach-O via `PROT_READ`/`MAP_PRIVATE` (lazy faults; no `MAP_POPULATE`); fallback `fs::read`. Parse only touches headers/LC → **`parse_main` ~11 ms** (was ~170–210 ms with full `Vec` read). Warm `clang --version` load **~0.27 s** (`mmap_main` ~140 ms, bind ~60 ms, `patch_svc` ~45 ms). g4-mini modern+flto **~2.3 s** (was ~3.6 s / originally ~6.5 s). Nested re-exec inherits `KAKEHASHI_LOAD_TIMING`. Next: process model; optional faster `patch_svc` / file-backed guest maps |
| File-backed guest segments + faster svc scan | kh-runtime | **landed** (2026-08, **general**). (1) `map_one`: host-page-aligned file interiors → `MAP_PRIVATE` file maps (CoW); only partial edge pages `read`. (2) `patch_svc`: unrolled LE word scan; mprotect still only on hits. Warm `clang --version` load **~0.19–0.21 s** (`mmap_main` ~78 ms, `patch_svc` ~29 ms, bind ~70 ms, parse ~9 ms). g4-mini modern+flto **~1.8 s**. Fixtures + product PASS. Next: process model (nested `kh`); bind cost; optional skip icache on pure file maps |
| Bind resolve cache (export index + once flat map) | kh-loader | **landed** (2026-08, **general**). Per-site `lookup_flat` rebuilt a full export `HashMap` (string clones) and `lookup_in_image` scanned nlist exports linearly — O(sites × exports) for any chained/classic guest. Fix: `export_by_name` on each image after `fill_exports`; process-wide `BindResolveCache` (flat map, install-name / basename / bottle-alias index, dep ordinal lists, missing-stub handlers) built once per `bind_process`. Docker warm `clang --version` **~0.19–0.20 s** (`mmap_main` ~84 ms, `bind_fill_exports` ~36 ms, `bind_chained` ~28 ms, `bind_resolve_cache` ~0.1 ms, `patch_svc` ~30 ms). g4-mini modern+flto **~1.5–1.6 s** (was ~1.8 s), product `g4-mini PASS`. `docker-smoke` green. Residual bind cost is mostly nlist scan of huge mains (`fill_exports`); next: cheaper export fill / process model |
| Dysymtab-bounded export/import nlist walk | kh-loader | **landed** (2026-08, **general**). `defined_exports` / `undefined_imports` used to walk **all** nlist entries. CLT `clang` has ~206k locals and **`nextdefsym == 0`** — full walk was pure waste (~35 ms). When trustworthy `LC_DYSYMTAB` is present, walk only `iextdefsym..+nextdefsym` / `iundefsym..+nundefsym` (empty extdef → return immediately). All-zero vacuous dysymtab (some fixtures) falls back to full walk. Docker warm `clang --version` **~0.14 s** (`mmap_main` ~80 ms, **`bind_fill_exports` ~0.2 ms** (was ~36 ms), `bind_chained` ~14–22 ms, `patch_svc` ~29 ms). g4-mini modern+flto **~1.2–1.4 s**, product PASS. Next: `mmap_main` / `patch_svc` / process model |
| Pure file RX map + selective I-cache + parallel svc scan | kh-runtime | **landed** (2026-08, **general**). (1) Host-page-aligned segments whose file bytes cover the full host map → single `MAP_PRIVATE` file map with **final** prot (no anon+overlay). (2) `__clear_cache` only after userspace stores (edge fills / residual patches) — pure file maps stay demand-paged (was bulk-flush of ~100 MiB TEXT). (3) `patch_svc`: parallel scan for ≥8 MiB RX; flush I-cache only on hit sites. Docker warm `clang --version` **~0.07 s** (`mmap_main` **~0.4 ms**, `patch_svc` ~44 ms, `bind_chained` ~13 ms). g4-mini modern+flto **~0.8–1.0 s**, product PASS; fixtures green. Residual: residual-`svc` scan of large TEXT; process model |
| Instruction-section-only residual `svc` scan | kh-loader / kh-runtime | **landed** (2026-08, **general**, **no bottle cache**). Plan emits `svc_scan_ranges` from sections with `S_ATTR_PURE_INSTRUCTIONS` / `S_ATTR_SOME_INSTRUCTIONS` only; `patch_svc` walks those ranges (fallback: whole filesize if none). Fixes false-positive rewrite of data that looks like `svc` in `__const`/`__cstring` (CLT `clang` had 6 full-TEXT “hits”, **0** in pure code). Docker warm `clang --version` **~0.08 s** (`patch_svc` ~50 ms still dominated by faulting ~93 MiB `__text` with 0 real hits; libSystem residual patch still correct). g4-mini flto **~0.78–0.94 s**, fixtures PASS. Explicitly **not** using on-disk hit-lists in the bottle. Next: process model / fewer nested `kh` |
| Direct host `posix_spawn` for nested `kh` (`KH_HELPER_SPAWN`) | freestanding + runtime | **landed** (2026-08, **general**, **no bottle files / no hit-list**). Guest freestanding `_posix_spawn` no longer `fork`+`execve`s the full guest image (CoW of CLT-sized maps before nested re-exec). Instead: hypercall `KH_HELPER_SPAWN` → host `posix_spawn` of `kh run <path> -- …` (same nested model, no parent address-space duplicate). Soft still ignores `file_actions`/`attrp`. Docker: g4-mini `-flto` **~0.77–0.99 s**, product PASS; warm `clang --version` **~0.065 s**. Residual wall is nested load+work (N× process start), not parent fork tax. |
