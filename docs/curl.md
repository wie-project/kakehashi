...
| **CA bundle (guest)** | `/etc/ssl/cert.pem` → bottle `private/etc/ssl/cert.pem` |
...
# → seeds Mozilla CA into private/etc/ssl/cert.pem
```
...
4. G4: OpenSSL TLS + bottle CA (`cert.pem`); host `VERIFY_CERT` helper; self-signed fails.
