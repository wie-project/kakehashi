...
| `futex` × large                     | ~95% strace time, ~257k calls         | guest park/wake; diagnose with `KAKEHASHI_FUTEX_STATS` |
...
| ID     | Idea                                                                 | Why                                                                 | Hard parts                                      |
| ------ | -------------------------------------------------------------------- | ------------------------------------------------------------------- | ----------------------------------------------- |
| **F1** | **Contended-bit mutex** (0/1/2; wake only if was 2)                  | ~~Always `FUTEX_WAKE` on unlock~~ **Done** — UTM: mutex residual tiny (`exp2`~59) | Validated via `KAKEHASHI_FUTEX_STATS`            |
| **F1c**| **Cond: waiter count + real signal**                                 | UTM: `other(cond)`~21k park, `n=broad`~46k, `woken0`~25k — signal≡broadcast + always-wake | Stage dylib; gen always bumps; WAKE only if nwaiters>0; signal wakes 1 |
| **F2** | Adaptive spin / pause tuning                                         | Secondary after F1c                                                 | Don't reintroduce yield-SVC storms              |
| **F3** | Heap freestanding lock → same 0/1/2 if CPU-bound spin shows up       | After F1c validated                                                 | Heap is pure spin today                         |
...
| **2026-07** | **F1c** cond signal/broadcast                                         | **Done** (code) — `pthread_cond`: word0=gen, word1=nwaiters; always `gen++`; `FUTEX_WAKE` only if nwaiters>0; **signal wakes 1**, broadcast wakes all (was signal≡broadcast always-wake). Stage dylib required. |
